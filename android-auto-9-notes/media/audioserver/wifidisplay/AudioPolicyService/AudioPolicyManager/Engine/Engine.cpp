...
audio_devices_t Engine::getDeviceForStrategy(routing_strategy strategy) const
{
    ...
    /**
     * 获取 AudioPolicyManager::mOutputs
    */
...
audio_devices_t Engine::getDeviceForStrategyInt(routing_strategy strategy, //STRATEGY_MEDIA
        DeviceVector availableOutputDevices,
        DeviceVector availableInputDevices,
        const SwAudioOutputCollection &outputs,
        uint32_t outputDeviceTypesToIgnore /*= AUDIO_DEVICE_NONE*/) const
{
    ...
    case STRATEGY_MEDIA: {   // &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&7
        ...
        if ((strategy == STRATEGY_MEDIA) &&
            (mForceUse[AUDIO_POLICY_FORCE_FOR_HDMI_SYSTEM_AUDIO] == AUDIO_POLICY_FORCE_HDMI_SYSTEM_AUDIO_ENFORCED)) {
            device &= ~AUDIO_DEVICE_OUT_SPEAKER;
        }
