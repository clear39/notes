...
/**
 * 这里是策略配置文件解析入口
 * 在AudioPolicyManager::loadConfig() --> deserializeAudioPolicyXmlConfig 中调用
*/
...
//  deserializeCollection<RouteTraits>(doc, root, routes, module.get());
static status_t deserializeCollection(_xmlDoc *doc, const _xmlNode *cur,
                                      typename RouteTraits::Collection &collection,
                                      typename RouteTraits::PtrSerializingCtx serializingContext)
{
    const xmlNode *root = cur->xmlChildrenNode;
    while (root != NULL) {
        ...
        const xmlNode *child = root;
        if (!xmlStrcmp(child->name, (const xmlChar *)Trait::collectionTag)) {// RouteTraits::collectionTag = "routes";
            child = child->xmlChildrenNode;
        }
        while (child != NULL) {
            if (!xmlStrcmp(child->name, (const xmlChar *)Trait::tag)) {//   RouteTraits::tag = "route";
                //  typedef sp<AudioRoute> PtrElement;
                typename Trait::PtrElement element;

                //  RouteTraits::deserialize(doc, child, element, serializingContext);
                status_t status = Trait::deserialize(doc, child, element, serializingContext);
                if (status != NO_ERROR) {
                    return status;
                }
...
//  RouteTraits::deserialize(doc, child, element, serializingContext);
status_t RouteTraits::deserialize(_xmlDoc /*doc*/, const _xmlNode *root, sp<AudioRoute> &element,PtrSerializingCtx ctx)
{
    string type = getXmlAttribute(root, Attributes::type); // const char RouteTraits::Attributes::type[] = "type";
    if (type.empty()) {
    ...
    string sinkAttr = getXmlAttribute(root, Attributes::sink);//const char RouteTraits::Attributes::sink[] = "sink";
    if (sinkAttr.empty()) {
    ...
    */
    ...
    string sourcesAttr = getXmlAttribute(root, Attributes::sources); // RouteTraits::Attributes::sources[] = "sources";
    ...
    */
    ...
    /**
     * 注意 这里sources 是通过 source 属性值查找到的AudioPort， 设置添加Route
    */
