...
android_media_AudioSystem_listAudioPorts(JNIEnv *env, jobject clazz,jobject jPorts, jintArray jGeneration)
{
    ...
    do {
        ...
        numPorts = 0;
        /**
         * 注意这里第四个参数为NULL，表示只是统计当前多少个AudioPort
         * 
         * 会调用 AudioPolicyService 的 listAudioPorts 方法
        */
        ...
        nPorts = (struct audio_port *)realloc(nPorts, numPorts * sizeof(struct audio_port));
         ...
         /**
         * 注意这里第四个参数不为NULL，表示获取对应的AudioPort
         * 
         * 会调用 AudioPolicyService 的 listAudioPorts 方法
        */
...
android_media_AudioSystem_listAudioPatches(JNIEnv *env, jobject clazz,jobject jPatches, jintArray jGeneration)
{
    ...
    do {
        ...
        ALOGV("listAudioPatches AudioSystem::listAudioPatches numPatches %d generation %d generation1 %d",numPatches, generation, generation1);
    ...
    for (size_t i = 0; i < numPatches; i++) {
        ...
        ALOGV("listAudioPatches patch %zu num_sources %d num_sinks %d",i, nPatches[i].num_sources, nPatches[i].num_sinks);
        ...
        for (size_t j = 0; j < nPatches[i].num_sources; j++) {
            ...
            jSource = NULL;
            ALOGV("listAudioPatches patch %zu source %zu is a %s handle %d",i, j,nPatches[i].sources[j].type == AUDIO_PORT_TYPE_DEVICE ? "device" : "mix",nPatches[i].sources[j].id);
        }
        // load sinks
        ...
        for (size_t j = 0; j < nPatches[i].num_sinks; j++) {
            jStatus = convertAudioPortConfigFromNative(env,NULL,&jSink,&nPatches[i].sinks[j]);
            ...
            jSink = NULL;
            ALOGV("listAudioPatches patch %zu sink %zu is a %s handle %d",i, j,nPatches[i].sinks[j].type == AUDIO_PORT_TYPE_DEVICE ? "device" : "mix",nPatches[i].sinks[j].id);
        }
