 ...
 */
	...
	for (i = 0; i < NUM_DATAPOINTS; i++) {
		VERB5 {
			ALOGV("datapoint[%d]: off:%f disp:%f(%f) age:%f%s",
				i,
				fdp[idx].d_offset,
	...
	ALOGE("filter offset:%+f disp:%f jitter:%f",
			p->filter_offset,
			p->filter_dispersion,
			p->filter_jitter);
}
...
reset_peer_stats(peer_t *p, double offset)
{
	...
	for (i = 0; i < NUM_DATAPOINTS; i++) {
		if (small_ofs) {
			...
			if (p->filter_datapoint[i].d_offset != 0) {
				...
				//ALOGE("p->filter_datapoint[%d].d_offset %f -> %f",
				//	i,
				//	p->filter_datapoint[i].d_offset + offset,
	...
	ALOGE("%s->lastpkt_recv_time=%f", p->p_dotted, p->lastpkt_recv_time);
}
...
resolve_peer_hostname(peer_t *p)
{
	...
	if (lsa) {
		...
		if (strcmp(p->p_hostname, p->p_dotted) != 0)
			ALOGE("'%s' is %s", p->p_hostname, p->p_dotted);
		p->dns_errors = 0;
		return lsa;
	}
	...
	if (resolve_peer_hostname(p)) {
		for (item = G.ntp_peers; item != NULL; item = item->link) {
			...
			if (pp->p_dotted && strcmp(p->p_dotted, pp->p_dotted) == 0) {
				ALOGE("duplicate peer %s (%s)", s, p->p_dotted);
				free(p->p_lsa);
				free(p->p_dotted);
				free(p);
				return;
...
#define PROBE_LOCAL_ADDR /* { len_and_sockaddr lsa; lsa.len = LSA_SIZEOF_SA; getsockname(p->query.fd, &lsa.u.sa, &lsa.len); } */
	...
	if (p->p_fd == -1) {
		 ...
		 */
		PROBE_LOCAL_ADDR
		bind(fd, &local_lsa->u.sa, local_lsa->len);
		PROBE_LOCAL_ADDR
	 ...
	 */
	ALOGV("sending query to %s", p->p_dotted);
...
static void run_script(const char *action, double offset)
{
	...
	ALOGV("executing '%s %s'", G.script_name, action);
...
step_time(double offset)
{
	...
	ALOGV("current time is %s.%06u", buf, (unsigned)tvc.tv_usec);
	
	tval = tvn.tv_sec;
	strftime_YYYYMMDDHHMMSS(buf, sizeof(buf), &tval);
	ALOGV("setting time to %s.%06u (offset %+fs)", buf, (unsigned)tvn.tv_usec, offset);
	...
	for (item = G.ntp_peers; item != NULL; item = item->link) {
		...
		//ALOGV("offset:%+f pp->next_action_time:%f -> %f",
...
fit(peer_t *p, double rd)
{
	if ((p->reachable_bits & (p->reachable_bits-1)) == 0) {
		...
		ALOGV("peer %s unfit for selection: "
...
select_and_cluster(void)
{
	...
	if (num_candidates == 0) {
		ALOGV("no valid datapoints%s", ", no peer selected");
		return NULL;
	}
//TODO: sorting does not seem to be done in reference code
	...
	while (1) {
		...
		if (num_falsetickers * 2 >= num_candidates) {
			ALOGV("falsetickers:%d, candidates:%d%s",
					num_falsetickers, num_candidates,
					", no peer selected");
			return NULL;
		}
	}
	ALOGV("selected interval: [%f, %f]; candidates:%d falsetickers:%d",
	...
	for (i = 0; i < num_points; i++) {
		...
		ALOGV("survivor[%d] metric:%f peer:%s", num_survivors, survivor[num_survivors].metric, p->p_dotted);
		num_survivors++;
	}
	...
	if (num_survivors < MIN_SELECTED) {
		ALOGV("survivors:%d%s",num_survivors,", no peer selected");
		return NULL;
	}
...
//we can avoid sorting then, just find the minimum once!
	...
	while (1) {
		...
		if (num_survivors <= MIN_CLUSTERED) {
			ALOGV("num_survivors %d <= %d, not discarding more",
		...
		for (i = 0; i < num_survivors; i++) {
			...
			ALOGV("survivor %d selection_jitter^2:%f",
					i, selection_jitter_sq);
		}
		max_selection_jitter = SQRT(max_selection_jitter / num_survivors);
		ALOGV("max_selection_jitter (at %d):%f min_jitter:%f",
		...
		if (max_selection_jitter < min_jitter) {
			ALOGV("max_selection_jitter:%f < min_jitter:%f, num_survivors:%d, not discarding more",
		 ...
		 * and go around again.
		 */
		ALOGV("dropping survivor %d", max_idx);
		num_survivors--;
 ...
 keep_old:
	ALOGV("selected peer %s filter_offset:%+f age:%f",
			p->p_dotted,
			p->filter_offset,
...
set_new_values(int disc_state, double offset, double recv_time)
{
	 ...
	 * the current time.
	 */
	ALOGV("disc_state=%d last update offset=%f recv_time=%f",
...
update_local_clock(peer_t *p)
{
	...
	if (abs_offset > PANIC_THRESHOLD) {
		ALOGV_and_die("offset %f far too big, exiting", offset);
	}
#endif
	...
	if (recv_time <= G.last_update_recv_time) {
		ALOGV("update from %s: same or older datapoint, not using it",
			p->p_dotted);
	...
	if (G.discipline_state == STATE_FREQ) {
		...
		if (since_last_update < WATCH_THRESHOLD) {
			ALOGV("measuring drift, datapoint ignored, %f sec remains",
...
// we end up here. Looks like _our_ clock is off.
		...
		case STATE_SYNC:
			...
			ALOGV("update from %s: offset:%+f, spike%s",
				p->p_dotted, offset,
				"");
		 ...
		 * intervals.
		 */
		ALOGV("stepping time by %+f; poll_exp=MINPOLL", offset);
		step_time(offset);
		...
		case STATE_NSET:
			...
			set_new_values(STATE_SYNC, offset, recv_time);
#endif
			ALOGV("transitioning to FREQ, datapoint ignored");
			return 0; /* "leave poll interval as is" */
	...
	ALOGV("updating leap/refid/reftime/rootdisp from peer %s", p->p_dotted);
	...
	ALOGV("discipline freq_drift=%.9f(int:%ld corr:%e) wander=%f",
	...
	if (adjtimex(&tmx) < 0)
		ALOGE("adjtimex");
	ALOGV("p adjtimex freq:%ld offset:%+ld status:0x%x tc:%ld",
...
//15:32:58.902 update from:<IP> offset:-0.000728 delay:0.158222 jitter:0.009454 clock drift:+9.298ppm tc:6
			 ...
			 */
			G.FREQHOLD_cnt = 1 + MIN_FREQHOLD + ((unsigned)(labs(tmx.offset)) >> 16);
	 ...
	 */
	ALOGV("adjtimex:%d freq:%ld offset:%+ld status:0x%x",
				rc, tmx.freq, tmx.offset, tmx.status);
	G.kernel_freq_drift = tmx.freq / 65536;
	ALOGV("update from:%s offset:%+f delay:%f jitter:%f clock drift:%+.3fppm tc:%d",
			p->p_dotted,
			offset,
			p->p_raw_delay,
			G.discipline_jitter,
...
poll_interval(int upper_bound)
{
	...
	ALOGV("chose poll interval:%u (poll_exp:%d)", interval, G.poll_exp);
	return interval;
}
static void
adjust_poll(int count)
{
	...
	if (G.polladj_count > POLLADJ_LIMIT) {
		...
		if (G.poll_exp < MAXPOLL) {
			G.poll_exp++;
			ALOGV("polladj: discipline_jitter:%f ++poll_exp=%d",
		...
		ALOGV("polladj: count:%d", G.polladj_count);
	}
}
static NOINLINE void
recv_and_process_peer_pkt(peer_t *p)
{
 ...
 recv_again:
	...
	if (size < 0) {
		 ...
		 */
		ALOGE("recv(%s) error", p->p_dotted);
	}

#if ENABLE_FEATURE_NTP_AUTH
	if (size != NTP_MSGSIZE_NOAUTH && size != NTP_MSGSIZE_MD5_AUTH && size != NTP_MSGSIZE_SHA1_AUTH) {
		ALOGV("malformed packet received from %s", p->p_dotted);
		return;
	}
	if (p->key_entry && hashes_differ(p, &msg)) {
		ALOGV("invalid cryptographic hash received from %s", p->p_dotted);
		return;
	}
#else
	...
	if ((msg.m_status & LI_ALARM) == LI_ALARM
	 ...
	 || msg.m_stratum > NTP_MAXSTRATUM
		...
		ALOGV("reply from %s: peer is unsynced", p->p_dotted);
		/*
...
//		return;                 /* invalid header values */
	...
	if (p->reachable_bits
	 ...
	 && delay > 1.0 / (8 * 1024) /* larger than ~0.000122 */
		...
		ALOGV("reply from %s: delay %f is too high, ignoring", p->p_dotted, delay);
		goto pick_normal_interval;
	}
	...
	ALOGV("%s->lastpkt_recv_time=%f", p->p_dotted, p->lastpkt_recv_time);
	...
	ALOGV("reply from %s: offset:%+f delay:%f status:0x%02x strat:%d refid:0x%08x rootdelay:%f reach:0x%02x",
		p->p_dotted,
		offset,
		p->p_raw_delay,
		p->lastpkt_status,
...
//BIGOFF check below deals with a case of seeing multi-second offsets.
			...
			if (fabs(q->filter_offset) >= POLLDOWN_OFFSET) {
				ALOGV("offset:%+f > POLLDOWN_OFFSET", q->filter_offset);
 ...
 increase_interval:
			...
			if (rc > 0)
				ALOGV("want smaller interval: offset/jitter = %u",
...
recv_and_process_client_pkt(void /*int fd*/)
{
...
#endif
	{
		...
		ALOGV("malformed packet received from %s: size %u", addr, (int)size);
		free(addr);
		goto bail;
	}
...
find_key_entry(llist_t *key_entries, unsigned id)
{
	...
	ALOGV_and_die("key %u is not defined", id);
}
#endif
...
static NOINLINE void ntp_init(char **argv)
{
	...
	if (getuid()){
		ALOGV("you must be root (uid:%d)",getuid());
	}
	...
	if (opts & OPT_k) {
		...
		while (config_read(parser, tokens, 4, 3, "# \t", PARSE_NORMAL | PARSE_MIN_DIE) == 3) {
			...
			if (strncasecmp(tokens[1], "sha", 3) == 0)
				...
				ALOGV_and_die("only MD5 and SHA1 keys supported");
/* man ntp.keys:
 ...
 */
 ...
 err:
				ALOGV_and_die("malformed key at line %u", parser->lineno);
			}
	...
	if (peers) {
		...
		ALOGV("ntp_init config_open /etc/ntp.conf");
...
# endif
			...
			ALOGV("skipping %s:%u: unimplemented command '%s'","/etc/ntp.conf", parser->lineno, token[0]
			);
		}
		config_close(parser);
	}
	...
	if (!(opts & OPT_n)) /* only if backgrounded: */{
		ALOGV("ntp_init write_pidfile_std_path_and_ext ");
...
#undef G
	...
	ALOGV(" bb_got_signal:%d cnt:%d",bb_got_signal,cnt);
	...
	while (!bb_got_signal) {
		...
		for (item = G.ntp_peers; item != NULL; item = item->link) {
			...
			if (p->next_action_time <= G.cur_time) {
				if (p->p_fd == -1) {
					...
					ALOGV("timed out waiting for %s, reach 0x%02x, next query in %us",p->p_dotted, p->reachable_bits, timeout);
		...
		ALOGV("poll:%us sockets:%u interval:%us", timeout, i, 1 << G.poll_exp);
...
int main(int argc , char **argv){
	ALOGV("NTP Server start");
	return ntpd_main(argc ,argv);
}
