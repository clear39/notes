...
int LogReader::getLogSocket() {
    ...
    if (sock < 0) {
        sock = socket_local_server(socketName, ANDROID_SOCKET_NAMESPACE_RESERVED, SOCK_SEQPACKET);
    }
...
bool LogReader::onDataAvailable(SocketClient* cli) {
    ...
    if (cp) {
        timeout = atol(cp + sizeof(_timeout) - 1) * NS_PER_SEC + log_time(CLOCK_REALTIME).nsec();
    }
    ...
    if (nonBlock && (sequence != log_time::EPOCH) && timeout) {
        class LogFindStart {  // A lambda by another name
           ...
           public:
            LogFindStart(pid_t pid, unsigned logMask, log_time& sequence, bool isMonotonic)
                : mPid(pid),
                  mLogMask(logMask),
                  mStartTimeSet(false),
                  mStart(sequence),
